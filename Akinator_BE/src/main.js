...
export function determineFeature(input, qna, algorithms) {
  ...
  if (addQuestionRegex.test(input)) {
    ...
    if (
      ...
      return "Masukan tanggal tidak valid";
    } else {
      return `Hari dari tanggal ${dateStr} adalah ${result}`;
    }
    ...
    if (
      ...
      return "Masukan ekspresi tidak valid";
    } else {
      return `The result is: ${result.toString()}`;
    }
  } else {
    // TODO : Change depending on the toggle in the web app
    return stringMatching.findMatchingString(input, qna, algorithms);
